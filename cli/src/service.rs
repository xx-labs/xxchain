...
pub fn new_partial_protonet(
	config: &Configuration,
) -> Result<sc_service::PartialComponents<
	FullClientProtonet, FullBackend, FullSelectChain,
	sc_consensus::DefaultImportQueue<Block, FullClientProtonet>,
	sc_transaction_pool::FullPool<Block, FullClientProtonet>,
	(
		impl Fn(
			node_rpc::DenyUnsafe,
			sc_rpc::SubscriptionTaskExecutor,
		...
		(
			sc_consensus_babe::BabeBlockImport<Block, FullClientProtonet, FullGrandpaBlockImportProtonet>,
			grandpa::LinkHalf<Block, FullClientProtonet, FullSelectChain>,
			sc_consensus_babe::BabeLink<Block>,
		),
		grandpa::SharedVoterState,
		Option<Telemetry>,
...
pub fn new_partial_phoenixx(
	config: &Configuration,
) -> Result<sc_service::PartialComponents<
	FullClientPhoenixx, FullBackend, FullSelectChain,
	sc_consensus::DefaultImportQueue<Block, FullClientPhoenixx>,
	sc_transaction_pool::FullPool<Block, FullClientPhoenixx>,
	(
		impl Fn(
			node_rpc::DenyUnsafe,
			sc_rpc::SubscriptionTaskExecutor,
		...
		(
			sc_consensus_babe::BabeBlockImport<Block, FullClientPhoenixx, FullGrandpaBlockImportPhoenixx>,
			grandpa::LinkHalf<Block, FullClientPhoenixx, FullSelectChain>,
			sc_consensus_babe::BabeLink<Block>,
		),
		grandpa::SharedVoterState,
		Option<Telemetry>,
...
pub fn new_full(
	...
	if config.chain_spec.is_phoenixx() {
		return new_full_base_phoenixx(config)
	}
	if config.chain_spec.is_protonet() {
		return new_full_base_protonet(config)
	}
	new_full_base(config)
}

/// Creates a full service from the configuration for xxnetwork runtime.
pub fn new_full_base(mut config: Configuration) -> Result<TaskManager, ServiceError> {
...
/// Creates a full service from the configuration for protonet runtime.
pub fn new_full_base_protonet(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	let sc_service::PartialComponents {
		client,
		backend,
		mut task_manager,
...
/// Creates a full service from the configuration for phoenixx runtime.
pub fn new_full_base_phoenixx(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	let sc_service::PartialComponents {
		client,
		backend,
		mut task_manager,
...
fn new_light_base(
	...
	let (client, backend, keystore_container, mut task_manager, on_demand) =
		sc_service::new_light_parts::<Block, XXNetworkRuntimeApi, _>(
			&config,
			telemetry.as_ref().map(|(_, telemetry)| telemetry.handle()),
			executor,
		)?;
...
fn new_light_base_protonet(
	...
	let (client, backend, keystore_container, mut task_manager, on_demand) =
		sc_service::new_light_parts::<Block, ProtonetRuntimeApi, _>(
			&config,
			telemetry.as_ref().map(|(_, telemetry)| telemetry.handle()),
			executor,
		)?;
...
fn new_light_base_phoenixx(
	...
	let (client, backend, keystore_container, mut task_manager, on_demand) =
		sc_service::new_light_parts::<Block, PhoenixxRuntimeApi, _>(
			&config,
			telemetry.as_ref().map(|(_, telemetry)| telemetry.handle()),
			executor,
		)?;
