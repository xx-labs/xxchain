...
fn basic_setup_works() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq_uvec!(
            ...
            vec![
                (31, ValidatorPrefs::<CmixHash>::default_with_hash(CmixHash::repeat_byte(31 as u8))),
                (21, ValidatorPrefs::<CmixHash>::default_with_hash(CmixHash::repeat_byte(21 as u8))),
                (11, ValidatorPrefs::<CmixHash>::default_with_hash(CmixHash::repeat_byte(11 as u8))),
            ]
        );
...
fn rewards_should_work() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            <Module<Test>>::reward_by_ids(vec![(a.stash, 50)]);
            <Module<Test>>::reward_by_ids(vec![(a.stash, 50)]);
            // This is the second validator of the current elected set.
            <Module<Test>>::reward_by_ids(vec![(b.stash, 50)]);
            ...
            assert_eq!(Balances::total_balance(&b.ctrl), init_a_ctrl);
            assert_eq!(Balances::total_balance(&a.stash), init_a_stash);
            assert_eq!(Balances::total_balance(&b.ctrl), init_b_ctrl);
            assert_eq!(Balances::total_balance(&b.stash), init_b_stash);
            assert_eq!(Balances::total_balance(&n.ctrl), init_n_ctrl);
            assert_eq!(Balances::total_balance(&n.stash), init_n_stash);
            assert_eq_uvec!(Session::validators(), vec![a.stash, b.stash]);
            assert_eq!(
                Staking::eras_reward_points(Staking::active_era().unwrap().index),
                EraRewardPoints {
                    total: 50 * 3 + 2,
                    individual: vec![(a.stash, 100 + 1), (b.stash, 50 + 1)].into_iter().collect(),
                }
            );

            let part_for_a = Perbill::from_rational::<u32>(1000, 1125);
            let part_for_b = Perbill::from_rational::<u32>(1000, 1375);
            let part_for_n_from_a = Perbill::from_rational::<u32>(125, 1125);
            let part_for_n_from_b = Perbill::from_rational::<u32>(375, 1375);
            ...
            assert_eq_error_rate!(
                ...
                init_a_stash + part_for_a * total_payout_0 * 101 / 152,
                2,
            );
            assert_eq!(Balances::total_balance(&a.ctrl), init_a_ctrl);
            assert_eq_error_rate!(
                Balances::total_balance(&b.stash),
                init_b_stash + part_for_b * total_payout_0 * 51 / 152,
                2,
            );
            assert_eq!(Balances::total_balance(&b.ctrl), init_b_ctrl);
            assert_eq_error_rate!(
                ...
                init_n_stash
                    + part_for_n_from_a * total_payout_0 * 101 / 152
                    + part_for_n_from_b * total_payout_0 * 51 / 152,
                2
            );
            assert_eq!(Balances::total_balance(&n.ctrl), init_n_ctrl);
            ...
            // this time around all rewards go to A and N as their nominator
            <Module<Test>>::reward_by_ids(vec![(a.stash, 1)]);
            ...
            assert_eq_error_rate!(
                Balances::total_balance(&a.stash),
                init_a_stash + part_for_a * (total_payout_0 * 101 / 152 + total_payout_1),
                2,
            );
            assert_eq!(Balances::total_balance(&a.ctrl), init_a_ctrl);
            assert_eq_error_rate!(
            ...
            assert_eq_error_rate!(
                Balances::total_balance(&n.stash),
                init_n_stash
                    + part_for_n_from_a * (total_payout_0 * 101 / 152 + total_payout_1)
                    + part_for_n_from_b * total_payout_0 * 51 / 152,
                2
            );
            assert_eq!(Balances::total_balance(&n.ctrl), init_n_ctrl);
...
fn staking_should_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_ok!(Staking::bond(Origin::signed(3), 4, 1500));
            assert_ok!(Staking::validate(
                Origin::signed(4),
                ValidatorPrefs::default()
            ));
...
fn nominating_and_rewards_should_work() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            mock::make_all_reward_payment(0);
            assert_eq!(Balances::total_balance(&41), 2000 + total_payout_0 / 2);
            assert_eq!(Balances::total_balance(&31), 2000 + total_payout_0 / 2);
            ...
            let total_payout_1 = current_total_payout_for_duration(reward_time_per_era());
            <Module<Test>>::reward_by_ids(vec![(11, 1)]); // a gets 2 points
            <Module<Test>>::reward_by_ids(vec![(21, 2)]); // b gets 3 points
            // total points is 5
            ...
            mock::make_all_reward_payment(1);
            let payout_for_10 = 2 * total_payout_1 / 5;
            let payout_for_20 = 3 * total_payout_1 / 5;
...
fn validator_payment_prefs_work() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq_error_rate!(
            Balances::total_balance(&11),
            balance_era_1_11 + reward_of_11,
            2
        );
        assert_eq_error_rate!(
            Balances::total_balance(&101),
            balance_era_1_101 + reward_of_101,
            2
        );
    });
}
...
fn switching_roles() {
    ...
    ExtBuilder::default().nominate(false).build_and_execute(|| {
        ...
        assert_ok!(Staking::validate(
            Origin::signed(6),
            ValidatorPrefs::default_with_hash(CmixHash::repeat_byte(0x16))
        ));
        ...
        assert_ok!(Staking::validate(
            Origin::signed(2),
            ValidatorPrefs::default_with_hash(CmixHash::repeat_byte(0x12))
        ));
        // new stakes:
        // 10: 1000 self vote
...
fn bond_with_little_staked_value_bounded() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
        	...
        	// A: Validating (stash account: 1, controller account: 2)
        	// B: Validating (stash account: 11, controller account: 10)
        	// C: chilling
            ...
            // Stingy validator.
            assert_ok!(Staking::bond(Origin::signed(1), 2, 1));
            assert_ok!(Staking::validate(
                Origin::signed(2),
                ValidatorPrefs::default()
            ));
...
fn add_reward_points_fns_works() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        // accounts are assigned 1 extra point when receiving their first point of an era
        // so the reward point will be 1 greater for each account than expected
        ...
        assert_eq!(
            ErasRewardPoints::<Test>::get(Staking::active_era().unwrap().index),
            EraRewardPoints {
                individual: vec![(11, 5), (21, 3)].into_iter().collect(),
                total: 8,
            },
        );
    })
...
fn six_session_delay() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(Staking::eras_reward_points(init_active_era).total, 2); // 1 plus 1 for initialization
...
fn test_max_nominator_rewarded_per_validator_and_cant_steal_someone_else_reward() {
    ExtBuilder::default().build_and_execute(|| {
        for i in 0..=<Test as Config>::MaxNominatorRewardedPerValidator::get() {
            ...
            assert_ok!(Staking::bond(Origin::signed(stash), controller, balance));
...
fn chill_other_works() {
    ...
    ExtBuilder::default()
        ...
        .min_bond(min_bond)
        .build_and_execute(|| {
            // chill other should target controller accounts only
            assert_noop!(
                Staking::chill_other(
                    Origin::signed(1337),
                    11
            ...
            // can't chill other if account is a nominator
            assert_noop!(
                Staking::chill_other(
                    Origin::signed(1337),
                    100
            ...
            // can't chill other if account is a validator that meets minimum bond
            assert_noop!(
                Staking::chill_other(
                    Origin::signed(1337),
                    10
            ...
            // can always chill own account
            assert_ok!(Staking::chill_other(
                Origin::signed(30),
                30
            ));

            // increase minimum validator bond
            assert_ok!(Staking::set_validator_min_bond(
                Origin::root(),
                min_bond_increased
            ));

            // can chill validator with lower stake than new minimum
            assert_ok!(Staking::chill_other(
                Origin::signed(1337),
                10
            ));

            // still can't chill validator that meets minimum bond
            assert_ok!(Staking::chill_other(
                Origin::signed(1337),
                20
            ));
