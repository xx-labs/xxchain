	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn unbond(origin, #[compact] value: BalanceOf<T>) {
			...
			if !value.is_zero() {
				...
				// Make sure that the minimum validator bond is maintained
...
impl<T: Config> Module<T> {
	...
	fn do_payout_stakers(validator_stash: T::AccountId, era: EraIndex) -> DispatchResultWithPostInfo {
		...
		if let Some(imbalance) = Self::make_payout(
			...
			Self::deposit_event(RawEvent::Reward(ledger.stash, amount));
		}
		...
		for nominator in exposure.others.iter() {
			...
			if let Some(imbalance) = Self::make_payout(&nominator.who, nominator_reward) {
				...
				let amount = imbalance.peek();
				T::Reward::on_unbalanced(imbalance);
				Self::deposit_event(RawEvent::Reward(nominator.who.clone(), amount));
			}
		}
	...
	fn remove_validator(stash: &T::AccountId) {
		let prefs = <Validators<T>>::take(stash);
	...
	for Module<T>
{
	...
	fn put_snapshot(
		...
		targets.into_iter().for_each(|v| {
			...
			<CmixIds<T>>::insert(&T::Hash::default(), ());
		});
