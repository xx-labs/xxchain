...
//! The xx network EVM precompiles. This can be compiled with ``#[no_std]`, ready for Wasm.
...
/// The PrecompileSet installed in the xx network runtime.
#[derive(Debug, Default, Clone, Copy)]
pub struct XXNetworkPrecompiles<R>(PhantomData<R>);

impl<R> XXNetworkPrecompiles<R> {
    pub fn new() -> Self {
    ...
    pub fn used_addresses() -> impl Iterator<Item = H160> {
        sp_std::vec![1, 2, 3, 4, 5, 6, 7, 8, 1024, 1025, 1026, 1027, 20482, 20483]
            .into_iter()
            .map(hash)
    }
}
...
impl<R> PrecompileSet for XXNetworkPrecompiles<R>
where
